...
class PotViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if currency_id:
            try:
                ...
                for pot in pots:
                    pot.amount = Currency.convert(
                        pot.amount, pot.currency, currency)
            except:
                pass
    ...
    def networth(self, request, *args, **kwargs) -> Response:
        ...
        for networth_bundle in networth_list:
            serialized_currency: CurrencySerializer = CurrencySerializer(
                networth_bundle['currency'], many=False).data
    ...
    def networth_range(self, request, *args, **kwargs) -> Response:
        ...
        for currency in Currency.objects.all().filter(user=self.request.user):
            ...
            # cycles through each pot and gets it's own range of {dates, amounts} with amounts converted to the index currency
            for pot in pots:
                range_dict_for_pot: dict = Record.fetch_pot_total_from(
                    pot, from_date, granularity)
                range_dict_for_pot['amounts'] = Currency.convert_list(
                    range_dict_for_pot['amounts'], pot.currency, currency)
                all_pots_ranges.append(range_dict_for_pot)
            amounts_super_list: list = [
                range_dictionary['amounts'] for range_dictionary in all_pots_ranges]
            zipped_list: list = list(zip(*amounts_super_list))
            # LOLOLOL Not a very readable line but efficient....
            amounts_list = [sum([Decimal(item) for item in tup])
    ...
    def record_range(self, request, pk=None, *args, **kwargs) -> Response:
        ...
        data = Record.fetch_pot_total_from(
            Pot.objects.get(id=pk), from_date, granularity)
    ...
    def create(self, request, *args, **kwargs) -> Response:
        ...
        try:
            currency: Currency = Currency.objects.get(
                id=currency_id, user=request.user)
        except Exception:
    ...
    def partial_update(self, request, pk=None, *args, **kwargs) -> Response:
        ...
        new_amount = self.request.data['amount']
        currency_id = self.request.data['currency']

        if currency_id:
            try:
                pot.currency = Currency.objects.get(id=currency_id)
                pot.save()
            except:
                pass
        ...
        if new_amount and new_amount != pot.amount:
            if new_amount > pot.amount:
                trans: Transaction = Inflow.objects.create(
                    user=request.user,
                    title="deposit",
                ...
                trans: Transaction = Outflow.objects.create(
                    user=request.user,
                    title="withdrawal",
